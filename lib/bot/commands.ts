...
export async function processCommand(command: string, chatId: number, user: any) {
  ...
  switch (commandName) {
    ...
    default:
      return await sendMessage(chatId, "Неизвестная команда. Используйте /help для справки.")
  }
}
...
async function handleStart(chatId: number, user: any) {
  ...
  const { data: shops } = await supabase.from("shop_admins").select("shops(*)").eq("telegram_id", user.id)

  if (!shops || shops.length === 0) {
    return await sendMessage(chatId, "Добро пожаловать! У вас пока нет магазинов. Создайте магазин в веб-админке.")
  }
...
async function handleStats(chatId: number) {
  ...
  const { data: shops } = await supabase.from("shop_admins").select("shop_id").eq("telegram_id", chatId)
  ...
  const { data: orders, error: ordersError } = await supabase.from("orders").select("*").in("shop_id", shopIds)

  if (ordersError) {
    console.error("Ошибка получения заказов:", ordersError)
    return await sendMessage(chatId, "Произошла ошибка при получении статистики.")
  }

  // Получаем статистику по товарам
  const { data: products, error: productsError } = await supabase.from("products").select("*").in("shop_id", shopIds)
...
async function handleOrders(chatId: number, args: string[]) {
  ...
  message += "Для управления заказом используйте команду:\n/orders <id_заказа> <действие>\n"
...
async function handleProducts(chatId: number, args: string[]) {
  ...
  if (!products || products.length === 0) {
    return await sendMessage(chatId, "У вас пока нет товаров. Используйте /addproduct для добавления.")
  }
  ...
  message += "Для управления товаром используйте команду:\n/products <id_товара> <действие>\n"
