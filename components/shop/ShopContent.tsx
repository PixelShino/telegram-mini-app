...
import { ArrowLeft, Plus, Minus, Trash2, Store, ShoppingBag, Package, CheckCircle } from "lucide-react"
...
export default function ShopContent({ shop, telegramUser, isTelegram }: ShopContentProps) {
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)
  const [currentView, setCurrentView] = useState<"products" | "cart" | "order">("products")
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null)
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)

  const { items, addItem, removeItem, updateQuantity, getTotalPrice, getTotalItems, getItemQuantity, clearCart } =
    useCartStore()
    ...
    if (selectedCategory) {
      filtered = filtered.filter((product) => {
        ...
        if (selectedCategory === "electronics") {
          if (selectedSubcategory === "phones") return product.name.includes("iPhone")
          if (selectedSubcategory === "tablets") return product.name.includes("iPad")
          if (selectedSubcategory === "smart-devices") return product.name.includes("Watch")
          return product.name.includes("iPhone") || product.name.includes("iPad") || product.name.includes("Watch")
        }
        if (selectedCategory === "computers") {
          if (selectedSubcategory === "laptops") return product.name.includes("MacBook")
          return product.name.includes("MacBook")
        }
        if (selectedCategory === "accessories") {
          if (selectedSubcategory === "audio") return product.name.includes("AirPods")
          if (selectedSubcategory === "input") return product.name.includes("Keyboard")
          return product.name.includes("AirPods") || product.name.includes("Keyboard")
        }
                        ...
                        <div className="flex flex-col items-end gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                if (product.allow_quantity_change || quantity === 0) {
                                  updateQuantity(product.id, quantity + 1)
                                  toast.success(`Добавлено еще: ${product.name}`, {
                              ...
                              className="h-8 w-8 p-0"
                              disabled={!product.allow_quantity_change && quantity >= 1}
                            >
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="space-y-3">
                    ...
                    <Button onClick={() => setCurrentView("order")} className="w-full h-12">
                      Оформить заказ
                    </Button>
                  </div>
                </CardContent>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-[96px] pb-24 px-4 max-w-md mx-auto">
        {/* Заголовок магазина */}
        <header className="text-center mb-6 space-y-2 pt-4">
          <h1 className="text-2xl font-bold">{shop.name}</h1>
          {shop.description && <p className="text-muted-foreground">{shop.description}</p>}
          {telegramUser.first_name && <p className="text-sm text-primary">Привет, {telegramUser.first_name}!</p>}
        </header>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16 space-y-4">
            ...
            <h2 className="text-xl font-semibold">
              {searchQuery || selectedCategory ? "Товары не найдены" : "Товары скоро появятся"}
            </h2>
